...
if($_POST['type']){
	...
	switch ($type) {
		...
		case 4:
			...
			break ;
		case 5:
			mysqli_query($db_handle,"update projects set project_type= '$prjtype' where project_id='$knownid';") ;
			if($value != ""){
				$fundinfo = mysqli_query($db_handle, "select * from  project_funding_info where project_id = '$knownid' ;") ;
				if(mysqli_num_rows($fundinfo) == 0) {
					mysqli_query($db_handle, "INSERT INTO project_funding_info (project_id, project_value, fund_neede) VALUES ('$knownid', '$value', '$fund') ;") ;
				}
				else {
					mysqli_query($db_handle,"update project_funding_info set project_value = '$value', fund_neede = '$fund' where project_id='$knownid';") ;
				}
			}
			events($db_handle,$user_id,"11",$knownid) ;
			if(mysqli_error($db_handle)) { echo "Failed to Change"; }
