...
function user_articles ($db_handle, $user_id) {
    $user_articles_display = mysqli_query($db_handle, "(SELECT challenge_id, challenge_title, LEFT(stmt, 500) as stmt FROM challenges WHERE challenge_type=7 AND user_id=$user_id AND (challenge_status!=3 AND challenge_status!=7) AND blob_id=0)
                                                        UNION
                                                        (SELECT a.challenge_id, a.challenge_title, LEFT(b.stmt, 500) as stmt FROM challenges as a JOIN blobs as b WHERE a.challenge_type=7 AND a.user_id=$user_id AND (a.challenge_status!=3 AND a.challenge_status!=7) AND a.blob_id=b.blob_id) ORDER BY challenge_id DESC;");
    while($user_articles_displayRow= mysqli_fetch_array($user_articles_display)) {
        ...
        echo "<div class='col-md-12 text-left list-group-item'><br/>
...
function user_idea ($db_handle, $user_id) {
    $user_idea_display = mysqli_query($db_handle, "(SELECT challenge_id, challenge_title, LEFT(stmt, 200) as stmt FROM challenges WHERE challenge_type=4 AND user_id=$user_id AND (challenge_status!=3 AND challenge_status!=7) AND blob_id=0)
                                                        UNION
                                                        (SELECT a.challenge_id, a.challenge_title, LEFT(b.stmt, 200) as stmt FROM challenges as a JOIN blobs as b WHERE a.challenge_type=4 AND a.user_id=$user_id AND (a.challenge_status!=3 AND a.challenge_status!=7) AND a.blob_id=b.blob_id)ORDER BY challenge_id DESC;");
