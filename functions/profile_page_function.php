...
function user_articles ($db_handle, $user_id) {
    $user_articles_display = mysqli_query($db_handle, "(SELECT challenge_id, challenge_title, LEFT(stmt, 500) as stmt FROM challenges WHERE challenge_type=7 AND user_id=$user_id AND blob_id=0)
                                                        UNION
                                                        (SELECT a.challenge_id, a.challenge_title, LEFT(b.stmt, 500) as stmt FROM challenges as a JOIN blobs as b WHERE a.challenge_type=7 AND a.user_id=$user_id AND a.blob_id=b.blob_id) ORDER BY challenge_id DESC;");
...
function user_challenges ($db_handle, $user_id) {
    $user_challenges_display = mysqli_query($db_handle, "(SELECT a.challenge_id,a.challenge_title,b.user_id, LEFT(a.stmt, 200) as stmt FROM challenges as a JOIN challenge_ownership as b WHERE (a.challenge_type=1 OR a.challenge_type=3) AND a.user_id=$user_id AND b.user_id=$user_id AND a.blob_id=0 AND a.project_id=0)
                                                        UNION
                                                        (SELECT a.challenge_id,a.challenge_title,c.user_id, LEFT(b.stmt, 200) as stmt FROM challenges as a JOIN blobs as b JOIN challenge_ownership as c WHERE (a.challenge_type=1 OR a.challenge_type=3) AND a.user_id=$user_id AND c.user_id=$user_id AND a.blob_id=b.blob_id AND a.project_id=0) ORDER BY challenge_id DESC;");
...
function user_idea ($db_handle, $user_id) {
    $user_idea_display = mysqli_query($db_handle, "(SELECT challenge_id, challenge_title, LEFT(stmt, 200) as stmt FROM challenges WHERE challenge_type=4 AND user_id=$user_id AND blob_id=0)
                                                        UNION
                                                        (SELECT a.challenge_id, a.challenge_title, LEFT(b.stmt, 200) as stmt FROM challenges as a JOIN blobs as b WHERE a.challenge_type=4 AND a.user_id=$user_id AND a.blob_id=b.blob_id)ORDER BY challenge_id DESC;");
