...
      thisLengthLabel.addClass('control-label col-xs-12 col-sm-7 col-md-6').attr('for', currentId + '_length_select').css('text-align', 'left').css("margin-top","10px");
...
      thisLengthSelect.prependTo(thisLength).wrap('<div class="col-xs-12 col-sm-5 col-md-6" />');
      thisLengthSelect.css("margin","10px");

      // Re-arrange the search input for a form-horizontal layout
      thisFilter.addClass('form-group');
      thisFilterLabel.addClass('control-label col-xs-4 col-sm-3 col-md-3').attr('for', currentId + '_filter_input').css("margin-top","10px");
      thisFilterInput.addClass('form-control input-sm').attr('id', currentId + '_filter_input').css("margin-top","10px").css("padding-right","10px");
      thisFilterInput.appendTo(thisFilter).wrap('<div class="col-xs-8 col-sm-9 col-md-8 " />');
    }
  }
});
 
...
$(document).ready(function() {
    $('#datatable').dataTable( {
        ...
        "sPaginationType": "bootstrap",
        "oLanguage": {
            "sLengthMenu": "<i>Records per page</i> _MENU_ "
        }
    } );
});
